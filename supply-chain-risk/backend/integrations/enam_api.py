...
Fetches market price feeds from eNAM portal.
"""
import httpx
import logging
...
async def fetch_enam_prices(
    ...
    """
    Fetch market price data from eNAM API.
    eNAM provides integrated market price feeds across India.
    """
    try:
...
def _normalize_enam_data(records: List[Dict]) -> List[Dict[str, Any]]:
    """Normalize eNAM records to consistent format."""
    normalized = []
    for r in records:
        normalized.append({
            "source": "enam",
...
def _get_fallback_enam_data() -> List[Dict[str, Any]]:
    """Return structured fallback data when API is unavailable."""
    now = datetime.utcnow()
    return [
        {
            "source": "enam",
