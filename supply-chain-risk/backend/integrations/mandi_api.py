...
async def fetch_mandi_prices(
    ...
    """
    Fetch real-time commodity/mandi prices from data.gov.in
    Returns list of price records with market, commodity, and price data.
    """
    params = {
    ...
    try:
        ...
        logger.error(f"Mandi API HTTP error: {e.response.status_code}")
...
def _normalize_mandi_data(records: List[Dict]) -> List[Dict[str, Any]]:
    """Normalize raw mandi records to a consistent format."""
    normalized = []
    for r in records:
        normalized.append({
            "source": "mandi",
...
def _get_fallback_mandi_data() -> List[Dict[str, Any]]:
    """Return live-structure fallback data when API is unavailable."""
    return [
        {
            "source": "mandi",
            "state": "Maharashtra",
