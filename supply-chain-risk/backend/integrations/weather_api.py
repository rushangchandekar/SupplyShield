...
DISRUPTION_CONDITIONS = {
    "Thunderstorm": 0.8, "Rain": 0.4, "Heavy Rain": 0.7, "Drizzle": 0.1,
    "Snow": 0.6, "Mist": 0.2, "Fog": 0.4, "Haze": 0.2, "Dust": 0.3,
    "Tornado": 1.0, "Squall": 0.7, "Extreme": 0.9, "Clear": 0.0, "Clouds": 0.05,
}


async def fetch_weather_data(cities=None):
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            for hub in hubs:
                try:
                    params = {"lat": hub["lat"], "lon": hub["lng"], "appid": settings.WEATHER_API_KEY, "units": "metric"}
                    response = await client.get(f"{settings.WEATHER_API_URL}/weather", params=params)
...
def _normalize_weather(data, hub):
    ...
    temp_factor = 0.3 if (temp > 45 or temp < 0) else 0.0
    severity = min(base_severity + wind_factor + vis_factor + temp_factor, 1.0)
    return {
        "source": "weather", "city": hub["name"], "region": hub["region"],
        "lat": hub["lat"], "lng": hub["lng"], "weather_main": weather_main,
        "weather_description": weather_desc, "temperature": temp, "humidity": humidity,
        "wind_speed": wind_speed, "visibility": visibility,
        "disruption_severity": round(severity, 3), "is_disruptive": severity > 0.3,
        "timestamp": datetime.utcnow().isoformat()
    }


def _get_fallback_weather(hub):
    conditions = ["Clear", "Clouds", "Rain", "Haze", "Mist"]
    condition = random.choice(conditions)
    severity = DISRUPTION_CONDITIONS.get(condition, 0.0)
    return {
        "source": "weather", "city": hub["name"], "region": hub["region"],
        "lat": hub["lat"], "lng": hub["lng"], "weather_main": condition,
