    ...
    if (loading) {
        return (
            <div className="loading-container">
                ...
                <span className="loading-text">Analyzing supply chain signals...</span>
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div className="page-header">
                ...
                <p>
                    Real-time risk intelligence across procurement, transport, and trade •{' '}
            ...
            <div className="dashboard-grid animate-in">
                ...
                {Object.entries(segments).map(([key, seg], idx) => (
                    <div
                        key={key}
                        className={`glass-card segment-card animate-in animate-in-delay-${idx + 1}`}
                    >
                        <div className={`segment-icon segment-icon-${key}`}>
                            {SEGMENT_ICONS[key]}
                        </div>
                        ...
                        <div className="segment-bar">
                            <div
                                ...
                                style={{
                                    width: `${seg.score}%`,
                                    background: getRiskColor(seg.risk_level),
                                }}
                            />
                        </div>
                    </div>
            ...
            {/* Risk Trend Chart + Signals Summary */}
            <div className="dashboard-grid-2 animate-in animate-in-delay-2">
                ...
                <div className="glass-card">
                    ...
                    {data?.signals_summary && (
                        <div>
                            {[
                                { label: 'Mandi Prices', count: data.signals_summary.mandi_records, icon: '🌾', cls: 'signal-source-mandi' },
                                { label: 'eNAM Markets', count: data.signals_summary.enam_records, icon: '🏪', cls: 'signal-source-enam' },
                                { label: 'Weather Signals', count: data.signals_summary.weather_records, icon: '🌦️', cls: 'signal-source-weather' },
                                { label: 'Trade Data', count: data.signals_summary.trade_records, icon: '📊', cls: 'signal-source-trade' },
                                { label: 'Logistics', count: data.signals_summary.logistics_records, icon: '🚚', cls: 'signal-source-logistics' },
                            ].map((src, i) => (
                                <div key={i} className="signal-item">
                                    <div className={`signal-source-icon ${src.cls}`}>{src.icon}</div>
                        ...
                        data.bottlenecks.map((b, i) => (
                            <div key={i} className="bottleneck-card">
                                <div
                                    ...
                                    style={{
                                        background: `rgba(${b.combined_risk > 50 ? '244,63,94' : b.combined_risk > 30 ? '249,115,22' : '245,158,11'}, 0.15)`,
                                        color: b.combined_risk > 50 ? '#f43f5e' : b.combined_risk > 30 ? '#f97316' : '#f59e0b',
                                    }}
                                >
                                ...
                                <div className="bottleneck-info">
                                    ...
                                    <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                                        {Object.entries(b.factors || {}).map(([k, v]) => (
                                            <span key={k} className="recommendation-tag">
                                                {k}: {(v * 100).toFixed(0)}%
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <div className="recommendation-content">
                                ...
                                <div className="recommendation-meta">
                                    ...
                                    {rec.estimated_impact > 0 && (
                                        <span className="recommendation-tag">Impact: {rec.estimated_impact}%</span>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            <div className="glass-card animate-in animate-in-delay-4" style={{ marginBottom: 24 }}>
                ...
                <div style={{ height: 300 }}>
                    <ResponsiveContainer>
                        <BarChart
                            data={Object.entries(segments).map(([key, seg]) => {
                                const factors = seg.contributing_factors || {};
                                return {
                                    segment: SEGMENT_LABELS[key],
                                    ...Object.fromEntries(
                                        Object.entries(factors).map(([fk, fv]) => [fk, fv.contribution || 0])
                                    ),
                                };
                            })}
                            layout="vertical"
