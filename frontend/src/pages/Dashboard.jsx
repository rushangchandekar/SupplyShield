...
const SEGMENT_GRADIENTS = {
    procurement: 'linear-gradient(135deg, rgba(16,185,129,0.08), rgba(6,182,212,0.04))',
    transport: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(59,130,246,0.04))',
    import_export: 'linear-gradient(135deg, rgba(6,182,212,0.08), rgba(59,130,246,0.04))',
};
    ...
    return (
        <div>
            <div style={{ marginBottom: 28 }}>
                <div className="skeleton" style={{ width: 320, height: 36, marginBottom: 10 }} />
                <div className="skeleton" style={{ width: 480, height: 18 }} />
            </div>
    ...
    if (error) return (
        <div className="loading-container">
            <div style={{ textAlign: 'center' }}>
                ...
                <div className="error-message" style={{ maxWidth: 420 }}>{error}</div>
    ...
    return (
        <div>
            <div className="page-header animate-in">
                ...
                <p>Real-time risk intelligence across procurement, transport, and trade •{' '}
            ...
            {!isPremium && (
                <div className="premium-banner animate-in animate-in-delay-1" id="premium-upsell">
            ...
            <div className="dashboard-grid animate-in animate-in-delay-1">
                <div className="glass-card" style={{ gridColumn: 'span 1' }} id="overall-risk-card">
                    <div className="glass-card-header">
                        <span className="glass-card-title">Overall Risk</span>
                ...
                {Object.entries(segments).map(([key, seg], idx) => (
                    <div
                        key={key}
                        className={`glass-card segment-card animate-in animate-in-delay-${idx + 2}`}
                        style={{ background: SEGMENT_GRADIENTS[key] || undefined }}
                        id={`segment-card-${key}`}
                    >
                        <div className={`segment-icon segment-icon-${key}`}>{SEGMENT_ICONS[key]}</div>
                        <span className="segment-label">{SEGMENT_LABELS[key]}</span>
                        <div className={`segment-score ${getRiskClass(seg.score)}`}>
                            {seg.score.toFixed(1)}
                        </div>
                        ...
                        <div className="segment-bar">
                            <div className="segment-bar-fill" style={{
                                width: `${seg.score}%`,
                                background: `linear-gradient(90deg, ${getRiskColor(seg.risk_level)}, ${getRiskColor(seg.risk_level)}88)`
                            }} />
                        </div>
                    </div>
                ))}
            ...
            <div className="dashboard-grid-2 animate-in animate-in-delay-3">
                <div className="glass-card" id="risk-trend-card">
                    <div className="glass-card-header">
                        <span className="glass-card-title">📈 Risk Trend (14 Days)</span>
                    </div>
                    <div style={{ height: 300 }}>
                        <ResponsiveContainer>
                            <AreaChart data={trend}>
                                ...
                                <Legend />
                                <Area type="monotone" dataKey="overall" stroke="#3b82f6" fill="url(#colorOverall)" name="Overall" strokeWidth={2.5} />
                ...
                <div className="glass-card" id="data-sources-card">
                    <div className="glass-card-header">
                        <span className="glass-card-title">📡 Data Sources</span>
                        <span style={{
                            color: 'var(--accent-emerald)', fontSize: '0.75rem', fontWeight: 600,
                            display: 'flex', alignItems: 'center', gap: 5
                        }}>
                            <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--accent-emerald)', boxShadow: '0 0 8px rgba(16,185,129,0.5)', display: 'inline-block' }} />
                            Live
                        </span>
                    </div>
                    {data?.signals_summary && (
                        <div>
                            ...
                            <div style={{
                                ...
                                <div style={{ fontSize: '0.73rem', color: 'var(--text-muted)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Total Signals</div>
                                <div style={{ fontSize: '1.6rem', fontWeight: 900, letterSpacing: '-0.02em' }}>{data.signals_summary.total}</div>
                            </div>
                        </div>
                    )}
                </div>
            ...
            <div className="dashboard-grid-2 animate-in animate-in-delay-4">
                <div className="glass-card" id="bottlenecks-card">
                    <div className="glass-card-header">
                        <span className="glass-card-title">🔍 Identified Bottlenecks</span>
                    </div>
                    {data?.bottlenecks?.length > 0 ? data.bottlenecks.map((b, i) => (
                        <div key={i} className="bottleneck-card">
                            <div className="bottleneck-risk" style={{
                                background: `rgba(${b.combined_risk > 50 ? '244,63,94' : '249,115,22'}, 0.12)`,
                                color: b.combined_risk > 50 ? '#f43f5e' : '#f97316',
                                border: `1px solid rgba(${b.combined_risk > 50 ? '244,63,94' : '249,115,22'}, 0.18)`
                            }}>
                                {b.combined_risk.toFixed(0)}%
                            </div>
                            <div className="bottleneck-info">
                                <h4>{b.region}</h4>
                                <p>{b.explanations?.join(' • ') || `${b.signal_count} signals detected`}</p>
                            </div>
                        </div>
                    )) : <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', padding: '12px 0' }}>No significant bottlenecks detected. ✅</p>}
                </div>

                <div className="glass-card" id="recommendations-card">
                    <div className="glass-card-header">
                        <span className="glass-card-title">💡 Recommendations</span>
                    </div>
                            ...
                            <div className="recommendation-content">
                                ...
                                <div className="recommendation-meta">
                                    ...
                                    {rec.estimated_impact > 0 && (
                                        <span className="recommendation-tag" style={{ borderColor: 'rgba(16,185,129,0.2)', color: 'var(--accent-emerald)' }}>
            ...
            <div className="glass-card animate-in animate-in-delay-5" style={{ marginBottom: 24 }} id="risk-factors-card">
                <div className="glass-card-header">
                    <span className="glass-card-title">📊 Contributing Risk Factors (Explainability)</span>
                </div>
                <div style={{ height: 300 }}>
                    <ResponsiveContainer>
                        <BarChart data={Object.entries(segments).map(([key, seg]) => {
                            ...
                            return {
                                ...
                                ...Object.fromEntries(Object.entries(factors).map(([fk, fv]) => [fk, fv.contribution || 0]))
                            };
                        })} layout="vertical">
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis type="number" />
                            ...
                            <Tooltip content={<CustomTooltip />} />
                            <Legend />
                            <Bar dataKey="price_volatility" fill="#f43f5e" name="Price Volatility" stackId="a" radius={[0, 0, 0, 0]} />
                            ...
                            <Bar dataKey="trade_volume_change" fill="#3b82f6" name="Trade Volume" stackId="a" radius={[0, 4, 4, 0]} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
